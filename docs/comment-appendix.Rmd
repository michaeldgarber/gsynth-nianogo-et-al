...
Each observation in this dataset of weights is a state-year. Note that the weights add up to 1 over treated state-years.
```{r}
state_year_wts_overall
state_year_wts_overall %>% 
  mutate(dummy=1) %>% 
...
Confidence intervals can be calculated analogously, first calculating these measures in every bootstrap replicate and then taking the percentiles over replicates.
